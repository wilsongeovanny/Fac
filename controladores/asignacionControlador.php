	...
	public function agregar_asignacion_controlador(){
		if (!empty($_POST['codigo']) && !empty($_POST['codimg-up']) && !empty($_POST['codqr-up']) && !empty($_POST['pers-reg'])) {
			...
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
				"Texto"=>"Es necesario que haya seleccionado a una persona para la asignación, por favor selecciónela e intente nuevamente",
				"Tipo"=>"error"
			];
		}
			...
			//
						...
						if ($editarasignacion->rowCount()>=1) {
							...
							$alerta=[
								...
								"Texto"=>"La asignación del hardware a sido un exito",
								"Tipo"=>"success"
							];
							...
							$alerta=[
								"Alerta"=>"simple",
								"Titulo"=>"Ocurrió un error inesperado",
								"Texto"=>"Usted no ha realizado ningun cambio o su número de serie ya se encuentra registrado en el sistema, porfavor intente nuevamnete",
								"Tipo"=>"error"
							];
						}
						return mainModel::sweet_alert($alerta);
